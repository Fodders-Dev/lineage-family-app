...
## Ход выполнения (2026-08-27)

- **Шаг 1 — СДЕЛАН** (`903c18c`): `kMaxPostMedia = 30`. Живой прогон на
  эмуляторе поймал литерал, которого не было в списке файлов плана: пустое
  состояние медиа-карточки («Можно прикрепить до 5 файлов», `:1918`).
- **Шаги 2-3 — СДЕЛАНЫ** (`b6bcdc1`): пул из 4 воркеров с раскладкой по
  индексу выбора; провал файла валит публикацию (раньше фото молча
  выбрасывалось); модель `MediaUploadProgress` прокинута в `createPost`,
  кнопка публикации показывает «Загружено N из M» / «Публикуем…».
  4 юнит-теста (порядок при обратных задержках, пик ≤4, серия прогресса,
  отказ). Итог: analyze чист, 1417/1417.
- **Проверено вживую** (эмулятор, dev-flavor): 8 фото одной пачкой →
  счётчик «8/30» → публикация со стадией «Публикуем…» → пост с 8 фото;
  порядок подтверждён попиксельно (градиент цветов совпал позиция в позицию).
- **Правка к разделу рисков:** каждый файл уходит ОТДЕЛЬНЫМ запросом
  `/v1/media/upload` (`custom_api_storage_service.dart:110-129`), поэтому
  лимит тела 50 МБ действует ПОФАЙЛОВО и пачкой не набирается — предлагавшийся
  батч-предчек защищал бы от несуществующей угрозы. Реальный риск остаётся
  один: одиночный крупный ВИДЕО-файл.
- **Шаг 4 — СДЕЛАН** (`01a6aac`, `d61692d`): «Добавить фото» прямо в альбоме,
  общий пикер вынесен в `lib/services/gallery_media_picker.dart` (туда же
  переехал `kMaxPostMedia`), тест-сид `mediaPickerOverride`. Публикуется
  обычный пост с пустой подписью — альбом производный, бэкенд не тронут.
  3 виджет-теста. Живой прогон снова поймал то, чего тесты не видят:
  глобальная тема задаёт FAB'ам `CircleBorder` (extended-подпись торчала за
  круг → точечный `StadiumBorder`), а текст пустого состояния всё ещё звал
  «поделись первым моментом в ленте». Проверено e2e: 5 фото → секция месяца,
  порядок сохранён.
- **Шаг 5 — СДЕЛАН** (5a/5b/5c/5d): фоновая публикация с автоповтором.
  - 5a: механика ChatSendQueue извлечена в генерик `PendingSendQueue<T>`
    (`lib/services/pending_send_queue.dart`); чат стал тонким подклассом,
    `chat_send_queue_test.dart` (18) прошёл БЕЗ ЕДИНОЙ ПРАВКИ.
  - 5b: `PostPublishQueue` + `PendingPostPublish` (Hive, авто-ретрай при
    возврате сети, restore после kill; успех без серверного echo → элемент
    убирается сразу, лента будится через PostsRefreshCoordinator).
  - 5c: «Опубликовать» отпускает мгновенно; глобальный чип в shell
    (счётчик / «Повторить»+«Убрать» / исчезает после успеха).
  - 5d: два адверсариальных ревью (Fable-прогон: 9 подтверждённых,
    Sonnet-прогон: +2 уникальных, среди них P0) → исправлено всё:
    двойной тап = один пост (синхронный `_isLoading`-гвард);
    идемпотентный `clientRequestId` в POST /v1/posts (дедуп по автору на
    бэке — таймаут не отменяет «зомби»-запрос, ретрай больше не задваивает);
    очередь скоуплена по userId (смена аккаунта на общем устройстве не
    публикует чужие черновики); посты сериализованы (пул 4 воркеров не
    умножается); гонка restore-vs-enqueue закрыта per-key Future (чинит и
    латентную гонку чата); restore best-effort (без unhandled reject);
    профиль подписан на PostsRefreshCoordinator (координатор повышен до
    multi-subscriber); чип поднят над compose-баром/композером чата,
    uploading-вариант пропускает тапы, метка честна при нескольких упавших.
  - Проверено вживую (эмулятор): мгновенный pop + чип + автообновление
    ленты; оффлайн → failed-чип; возврат сети → автодоставка без тапа;
    двойной тап → ровно один пост на сервере.
  - Остаточные ограничения (осознанно): чип не виден на root-navigator
    экранах (composer/альбом/мобильный чат) — человек попадает в shell
    сразу после pop; альбомная загрузка (шаг 4) осталась inline — у неё
    свой прогресс на FAB и человек остаётся на экране.

## Пост-скриптум: бинарная загрузка (ночь 28→29.08)

Закрыт последний остаточный риск раздела выше: медиа уходили base64-JSON'ом
(+33% трафика, двойная память на строку, потолок express.json(50mb) для
видео). Теперь клиент шлёт сами байты в `PUT /v1/media/object` (поток с
потолком 64 МБ, тот же rate-limit-бакет), а при 404/405 от старого бэка
откатывается на base64 с запоминанием на сессию — порядок выкатки
клиент/бэк не важен (фолбэк проверен живьём против прод-бэка без роута).
Легаси-путь остаётся для клиентов до OTA.

## Ship-статус

Все 5 шагов готовы. Веб получает всё при push в main; Android-пользователям
нужен OTA-релиз (бамп `version:` в pubspec + `/release-ota`) — по явной
команде.
