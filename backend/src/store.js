...
class FileStore {
  ...
  _writableCirclesViewForTree(db, treeId, cache) {
    ...
    // SPEED-12 fix: `{...db}` here used to invoke EVERY own enumerable
    // property getter on the frozen snapshot, including the `sessions`
    // accessor that readSharedSnapshot()'s wrapper defines to always throw
    // (SPEED-11 invariant #2) — this overlay never hit that path before
    // SPEED-12 because listPosts was its only caller reachable with an
    // ACTUALLY frozen db, and listPosts used to call plain _read(). Now
    // that listPosts accepts readSharedSnapshot() too, copy keys
    // explicitly and skip `sessions` instead of spreading — mirrors
    // _buildSharedSnapshotView's own construction, not a workaround.
    const overlay = {};
